...
def get_download_path(location,
    ...
    try:
        if location:
            if thumbnail:
                location += '/TUBE2GO/.thumbnails'
                os.makedirs(location, exist_ok=True)
            elif process_dash_stream:
                location += '/TUBE2GO/.dash_stream'
                os.makedirs(location, exist_ok=True)
            elif download_video_path:
                location += '/TUBE2GO/videos'
                os.makedirs(location, exist_ok=True)
            elif download_playlist_video_path:
                location += '/TUBE2GO/playlist/videos'
                os.makedirs(location, exist_ok=True)
            elif download_playlist_audio_path:
                location += '/TUBE2GO/playlist/audio'
                os.makedirs(location, exist_ok=True)
            elif download_audio_path:
                location += '/TUBE2GO/audio'
                os.makedirs(location, exist_ok=True)
            else:
                location += '/Downloads'
            return location
            ...
            if HOME != '':
                if thumbnail:
                    HOME += '/Downloads/TUBE2GO/.thumbnails'
                    os.makedirs(HOME, exist_ok=True)
                elif process_dash_stream:
                    HOME += '/Downloads/TUBE2GO/.dash_stream'
                    os.makedirs(HOME, exist_ok=True)
                elif download_video_path:
                    HOME += '/Downloads/TUBE2GO/videos'
                    os.makedirs(HOME, exist_ok=True)
                elif download_audio_path:
                    HOME += '/Downloads/TUBE2GO/audio'
                    os.makedirs(HOME, exist_ok=True)
                else:
                    HOME += '/Downloads'
            else:
        ...
        HOME = QProcessEnvironment().systemEnvironment().value('SNAP_REAL_HOME') + "/Downloads/TUBE2GO/"
